...
const EquipmentForm: React.FC<EquipmentFormProps> = ({
  ...
  const renderLabelValue = (label: string, value?: string) => (
    <Col span={8} className="mb-4">
      ...
      <div className="border rounded p-2 bg-white min-h-[38px]">
        {value || "-"}
      </div>
    </Col>
  );
      ...
      >
        ...
        <Card className="bg-gray-50 p-4">
          ...
          {isEditing ? (
            ...
            <div className="mb-4">
              ...
              <div className="border rounded p-2 bg-white min-h-[38px] whitespace-pre-wrap">
        ...
        {!isEditing ? (
          <div className="space-y-4">
            {initialValues?.options?.map((opt: any, idx: number) => (
              <Card
                key={idx}
                title={`Option ${idx + 1}`}
              ...
              >
                ...
                <p>
                  <strong>Weight:</strong> {opt.weight} kg
                </p>
                <p>
                  <strong>Available:</strong> {opt.available}
                </p>
                {opt.primaryImage && (
                  <div className="mt-2">
                    ...
                    <img
                      ...
                      alt="Primary"
                      onClick={() =>
                        handleImageClick(opt.primaryImage.thumbnail || "")
                      }
                ...
                {opt.galleryImages?.length > 0 && (
                  <div className="mt-4 bg-gray-50 p-4 relative">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {opt.galleryImages.map((img: any, idx: number) => (
                        <img
                          key={idx}
                          src={img.thumbnail}
                          alt={`Gallery ${idx + 1}`}
            ...
            rules={[
              {
                validator: async (_, options) => {
                  if (!options || options.length < 1)
                    throw new Error("Please add at least 1 equipment option");
                },
              },
            ]}
          >
        ...
        {!isEditing ? (
          <MuscleGroupForm
            value={initialValues?.muscle_group_used}
            isEditing={false}
          />
        ) : (
        ...
        {!isEditing ? (
          <div className="space-y-2">
            {initialValues?.features?.map((feature: any, idx: number) => (
              <p key={idx}>• {feature.description}</p>
            ))}
          </div>
        ) : (
        ...
        {!isEditing ? (
          <div className="space-y-2">
            {initialValues?.additional_fields?.map(
              (field: AdditionalField, idx: number) => (
                <p key={idx}>
